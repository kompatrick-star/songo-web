      ...
      <div class="songo-wood-board-3d">
        ...
        <div class="epic-store-store store-nord">
          <div class="inner-hollow-cavity">
            ...
            <div 
              v-for="n in Math.min(scoreNord, 32)" 
              :key="'n-'+n" 
              class="metallic-gold-seed seed-nord" 
              :style="generateStoreGrainStyle(n)"
            ></div>
          </div>
        ...
        <div class="pits-grid-game">
          ...
          <div class="row-pits row-nord">
            <div 
              ...
              :key="index" 
              class="pit-interactive-box"
              :class="{ 'clickable-playable': tour === 'NORD' && plateau[index] > 0 }"
          ...
          <div class="row-pits row-sud">
            <div 
              v-for="(index, idx) in rangeeSudIndices" 
              :key="index" 
              class="pit-interactive-box"
              :class="{ 'clickable-playable': tour === 'SUD' && plateau[index] > 0 }"
              @click="jouerCase(index)"
            >
              <div class="pit-spherical-cavity">
                <div class="cavity-specular"></div>
        ...
        <div class="epic-store-store store-sud">
          <div class="inner-hollow-cavity">
            <div class="store-soft-glow"></div>
            <div 
              v-for="n in Math.min(scoreSud, 32)" 
              :key="'s-'+n" 
              class="metallic-gold-seed seed-sud" 
              :style="generateStoreGrainStyle(n)"
            ></div>
          </div>
          <div class="store-label-plaque">
    ...
    <div class="glass-dashboard bottom-panel">
      <div class="hud-log-section">
        ...
        <div class="terminal-list-log">
          <p v-for="(log, i) in history" :key="i" :class="{ 'neon-gold-text': i === history.length - 1 }">
            {{ log }}
          </p>
...
export default {
  data() {
    return {
      ...
      plateau: Array(14).fill(5), // 14 cases, 5 graines chacune
      tour: 'SUD',
      scoreSud: 0, 
      scoreNord: 0,
  ...
  methods: {
    jouerCase(index) {
      ...
      if (nbrPions === 0) return; // Case vide
      ...
      // 2. RETRAIT DES GRAINES
      this.plateau[index] = 0;
      let currentIdx = index;

      // 3. DISTRIBUTION (SEMIS) DANS LE SENS ANTI-HORAIRE
      while (nbrPions > 0) {
        currentIdx = (currentIdx + 1) % 14;
        
        // Règle Songo : on ne redistribue pas dans la case de départ
      ...
      // 4. GESTION DES CAPTURES (Règle simple: capture si case adverse à 2 ou 3)
      let grainesCapturees = 0;
      if (this.tour === 'SUD' && currentIdx >= 7 && currentIdx <= 13) {
        if (this.plateau[currentIdx] === 2 || this.plateau[currentIdx] === 3) {
          grainesCapturees = this.plateau[currentIdx];
    ...
    reinitialiserPartie() {
      ...
      this.history = [];
      this.lastMove = "";
      this.history.push(`[${new Date().toLocaleTimeString('fr-FR')}] 🔄 La partie a été réinitialisée.`);
    },
    generateGrainStyle(n) {
      ...
      return {
        transform: `translate(${x}px, ${y}px) scale(${1 - (n * 0.012)})`,
        zIndex: n
      };
    },
...
.victory-card h2 { color: #bf9b30; font-family: 'Cinzel', serif; }
.victory-card p { font-size: 1.2rem; color: #1a0f0a; margin: 20px 0; }
